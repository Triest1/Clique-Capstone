...
class AIDatasetService {
  ...
  async loadDataset() {
    ...
    try {
      console.log('Loading AI dataset from /chat-dataset/CliqueDataset.csv...')
      const response = await fetch('/chat-dataset/CliqueDataset.csv')
  ...
  findFuzzyMatch(query) {
    ...
    for (const entry of this.dataset) {
      ...
      if (score > bestScore && score > 0.1) { // Lowered threshold from 0.15 to 0.1
        bestScore = score
        bestMatch = entry
